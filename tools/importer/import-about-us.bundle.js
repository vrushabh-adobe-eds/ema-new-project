...
var CustomImportScript = (() => {
  ...
  function parse(element, { document }) {
    ...
    const headings = Array.from(document.querySelectorAll("h2")).filter((h) => !h.closest(XF_SEL));
    const nodes = [...headings, ...allXf].sort((a, b) => a.compareDocumentPosition(b) & Node.DOCUMENT_POSITION_FOLLOWING ? -1 : 1);
    const groups = [];
    let current = null;
    nodes.forEach((n) => {
      if (n.matches(XF_SEL)) {
